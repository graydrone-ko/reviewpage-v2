...
exports.registerValidation = [
    ...
    (0, express_validator_1.body)('role').isIn(['SELLER', 'CONSUMER', 'ADMIN']).withMessage('Role must be SELLER, CONSUMER, or ADMIN'),
    (0, express_validator_1.body)('birthDate').isLength({ min: 6, max: 6 }).withMessage('Birth date must be YYMMDD format'),
    (0, express_validator_1.body)('gender').isIn(['MALE', 'FEMALE']).withMessage('Gender must be MALE or FEMALE'),
    (0, express_validator_1.body)('phoneNumber').isLength({ min: 11, max: 11 }).withMessage('Phone number must be 11 digits'),
...
const register = async (req, res) => {
    try {
        ...
        // 전화번호 중복 검사
        const existingUserByPhone = await database_1.prisma.user.findUnique({
